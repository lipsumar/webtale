...
PageBuilder.prototype.toHandlebars = function(str) {
    ...
    if (vars.mobile_content === 'none') {
        delete vars.mobile_content
    } else if (!vars.mobile_content) {
...
function registerHelpers(){
    ...
    handlebars.registerHelper('link', toPath => {
        var toFile = path.join(options.from, toPath)
        if (!fs.existsSync(toFile + '.html')) {
            throw new Error(`Link to "${toPath}" but file ${toFile} doesn’t exist`);
        }
        self.emit('link', toPath);
        return options.base_url + toPath;
    })
}
